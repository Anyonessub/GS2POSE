...
def cal_NOCS(means3D, minp, maxp):

    means3D_normalized = (means3D-minp) / (maxp - minp)


    colors = means3D_normalized
...
def nocs_render(
    ...
    minp = means3D.min(0).values
    maxp = means3D.max(0).values
    print(minp, maxp)
    override_color = cal_NOCS(means3D, minp, maxp)
    colors_precomp = override_color
    ...
    if mask is not None:
        ...
        # print(viewpoint_camera.cam_trans_delta)
        rendered_image, radii, depth, opacity, n_touched = rasterizer(
            means3D=means3D,
            means2D=means2D,
            shs=shs,
