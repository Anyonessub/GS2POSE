...
class Refiner:
    ...
    def set_depth(self, depthpath, maskpath, depth_scale = 1.0):
        self.use_depth = True
    ...
    def train(self, viewpoint, gt_image):
        ...
        return render_pkg, np.linalg.inv(TP)
    ...
    def train_rgbd(self, viewpoint, gt_image, gt_depth):
        ...
        for tracking_itr in range(self.final):
            ...
            if (tracking_itr < self.first):
                ...
                loss_tracking = get_loss_tracking_rgbd(
                    image, gt_image ,depth, gt_depth, opacity
                )
                loss_tracking.backward()
        ...
        return render_pkg, viewpoint.get_T()
    ...
    def run(self, coarse_T, gt_image, gt_depth = None):
        ...
        if(gt_depth is not None):
            render_pkg, refine_T = self.train_rgbd(viewpoint, gt_image, gt_depth)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str)#配置文件
    ...
    plypath = "refine/000001/point_cloud/iteration_2000/point_cloud.ply"
    ...
    gt_image = gt_image.astype(np.float32) / 255.0  # 转换为 [0, 1] 范围
    ...
    refiner.set_depth(depthpath, maskpath)

    refine_T = refiner.run(coarse_T, gt_image)# 运行主程序
