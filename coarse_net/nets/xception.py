...
class Xception(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.block1(x)#h/2
        feature_layer_1 = self.block1.hook_layer
        x = self.block2(x)#h/2
        feature_layer_2 = self.block2.hook_layer
        x = self.block3(x)
        x = self.block4(x)
        x = self.block5(x)#c=728
        x = self.block6(x)
        x = self.block7(x)
        x = self.block8(x)
        x = self.block9(x)
        x = self.block10(x)
        feature_layer_3 = self.block10.hook_layer
        x = self.block11(x)
        x = self.block12(x)
        x = self.block13(x)
        x = self.block14(x)
        ...
        x = self.conv5(x)
        return feature_layer_1,feature_layer_2,feature_layer_3,x
...
# model=Xception(downsample_factor=8).cuda()
# img=Image.open('E:\Mars-seg data set\MSL\JPEGImages/121_0121ML0007580000103880E01_DXXX.jpg').convert('RGB')
